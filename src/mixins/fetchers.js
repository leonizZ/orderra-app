export default {
    methods: {
        ...
        getGame(resume = false) {
            this.getData(`/${window.SETTINGS.API_SLUG}/games?next_only=1`, response => {
        ...
        getSICredits(mobile, callback) {
            this.getData(`/${window.SETTINGS.API_SLUG}/sabong-int/account/${mobile}`, response => {
                if (callback) callback(response)
            })
        },
        getOutlets(mode) {
            this.getData(`/${window.SETTINGS.API_SLUG}/wallet/${mode}/outlets`, response => {
                response['mode'] = mode
                this.$store.dispatch('setOutlets', response)
            })
        },
        getWalletHistory(mode, page = 1, newData = false) {
            this.getData(`/${window.SETTINGS.API_SLUG}/wallet/${mode}?limit=2&page=${page}`, response => {
                response['mode'] = mode
                if (newData) response['new_data'] = true
